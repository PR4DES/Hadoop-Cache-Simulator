...
struct CompareEndTime {
	bool operator()(Container const a1, Container const a2) {
...
class ResourceManager {
	private:
		...
		priority_queue<Container, vector<Container>, CompareEndTime> event_queue;
	public:
...
int ResourceManager::JumpTime() {
	if(event_queue.empty()) {
		if(job_submit_queue.empty()) return -1;
		else return job_submit_queue.top()->submit_time;
	}
	while(event_queue.top().end_time == main_time && !event_queue.empty())
		event_queue.pop();
	if(!job_submit_queue.empty())
		if(job_submit_queue.top()->submit_time < event_queue.top().end_time)
			return job_submit_queue.top()->submit_time;
	return event_queue.top().end_time;
}
// for test print
...
/*	for(int i=0; i<namenode.GetFileNum(); i++) {
		for(int j=0; j<namenode.files[i]->GetFileSize(); j++) {
			cout << namenode.files[i]->GetData(j,0).GetNodePosition() << " " <<  namenode.files[i]->GetData(j,1).GetNodePosition() << " " <<  namenode.files[i]->GetData(j,2).GetNodePosition() << endl;
		}
	}
*/
	// 3. Job setting
	...
	while(1) {
		...
		int temp = resourcemanage.JumpTime();
		if(temp != -1) main_time = resourcemanage.JumpTime();
		else main_time++;
	}
